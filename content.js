...
function showNotification(message, severity='info') {
	...
	switch(severity) {
		case('success'):
			messageColour = '#28a745';
			break;
		case('error'):
			messageColour = '#FF0000';
			break;
		case('warning'):
			messageColour = '#FF9600';
			break;
		case('info'):
			messageColour = '#C8C8C8';
			break;
		default:
			messageColour = '#C8C8C8';
			break;
...
function pasteContents(message) {
	...
	if (inputElement) {
		...
		showNotification('Tracking ID added', 'success');
	 } 
	 else {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
	if (message.action === 'showNotification') {
			showNotification(message.text, message.severity); // Call the pasteContents function
	}
});
